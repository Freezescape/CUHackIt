#!/usr/bin/env python3
"""
YOLOv8 ONNX-based Security System for Raspberry Pi
- Properly handles YOLOv8 ONNX output format (N, 84)
- Treats det[4:] as class scores
- Avoids invalid class ID issues
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(LOG_DIR / 'yolov8_onnx_security_system.log'),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger(__name__)

# ── ONNX Model Class (YOLOv8 Format) ──────────────────────────────────────────
class YOLOv8ONNXDetector:
    def __init__(self, model_path):
        ...
        logger.info(f"✅ YOLOv8 ONNX model loaded: {model_path}")
    ...
    def postprocess(self, outputs, original_shape):
        """Postprocess YOLOv8 ONNX model outputs"""
        # YOLOv8 ONNX output format: [batch, num_detections, 84]
        # 84 = 4 (bbox) + 80 (class scores)
        
        detections = outputs[0]  # Get first batch
        
        # Force output into (N, 84) format
        if detections.ndim == 3:
            detections = detections[0]  # Remove batch dimension
        
        # Ensure we have the right number of columns (84)
        if detections.shape[1] != 84:
            logger.warning(f"⚠️  Unexpected output shape: {detections.shape}, expected (?, 84)")
            return []
        ...
        for detection in detections:
            ...
            class_scores = detection[4:]  # This is the YOLOv8 format
            
            # Get the class with highest score
            class_id = np.argmax(class_scores)
            class_score = float(class_scores[class_id])
            
            # For YOLOv8, we use the class score directly as confidence
            ...
            try:
                ...
                if len(indices) > 0:
                    ...
                    for i in indices:
                        ...
                        # Robust class name handling
                        if 0 <= class_id < len(CLASSES):
                            class_name = CLASSES[class_id]
                        else:
                            # Fallback for out-of-range class IDs
                            class_name = f"class_{class_id}"
                            logger.warning(f"⚠️  Class ID {class_id} out of range, using fallback name")
...
# ── Security System Class (YOLOv8 Format) ─────────────────────────────────────
class YOLOv8ONNXSecuritySystem:
    def __init__(self):
        self.detector = YOLOv8ONNXDetector(ONNX_MODEL_PATH)
        ...
        logger.info("YOLOv8 ONNX Security System initialized")
    
    def start(self):
        """Start the security system"""
        self.is_active = True
        logger.info("🚨 YOLOv8 ONNX Security System Activated")
        ...
        try:
            ...
            logger.info("🛑 YOLOv8 ONNX Security System Stopped by User")
            self.stop()
    ...
    def stop(self):
        ...
        GPIO.cleanup()
        logger.info("YOLOv8 ONNX Security System Stopped")
    
    def _camera_loop(self):
        """Main camera processing loop (YOLOv8 Format)"""
        ...
        logger.info(f"📹 Camera started (Index: {CAMERA_INDEX}) - YOLOv8 Format")
    ...
    def _create_incident_report(self, detections):
        ...
        incident = {
            ...
            'severity': "HIGH",
            'status': "ACTIVE",
            'source': "YOLOV8_ONNX_SECURITY_SYSTEM",
...
# ── Main Execution ────────────────────────────────────────────────────────────
def main():
    print("🚨 YOLOv8 ONNX Security System for Raspberry Pi")
    print("=" * 60)
    print("Features:")
    print("• Real-time human detection (YOLOv8 ONNX format)")
    print("• Buzzer alarm")
    print("• Video recording")
    print("• Proper YOLOv8 ONNX output handling")
    print("• Treats det[4:] as class scores")
    print("• Avoids invalid class ID issues")
    print("• Headless mode compatible")
    print("=" * 60)
    
    try:
        system = YOLOv8ONNXSecuritySystem()
        system.start()
    except Exception as e:
